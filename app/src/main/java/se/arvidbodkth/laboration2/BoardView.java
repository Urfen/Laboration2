        ...
        if(gameState == null){
            ...
            for (int i = 0; i < 7; i++) {
                ...
                for (int k = 0; k < 7-row; k++) {
                    ...
                    if(i == 0 || i == 6){
                        gamePieces.add(new BoardPieceView(super.getContext(),k*3*stepV, i*stepH,  (k*3*stepV+stepV) , (i*stepH+stepH),position++,controller.getColorOfPos(position)));
                    }
                    if(i == 1 || i == 5){
                        gamePieces.add(new BoardPieceView(super.getContext(), (k * 2 + 1) * stepV, i * stepH, ((k * 2 + 1) * stepV + stepV), (i * stepH + stepH),position++,controller.getColorOfPos(position)));
                    }
                    if(i == 2 || i == 4){
                        gamePieces.add(new BoardPieceView(super.getContext(), (k+2)*stepV, i*stepH,  ((k+2)*stepV+stepV) , (i*stepH+stepH),position++,controller.getColorOfPos(position)));
                    }
                    if(i == 3) {
                        if(k != 3) gamePieces.add(new BoardPieceView(super.getContext(), (k)*stepV, i*stepH,  ((k)*stepV+stepV) , (i*stepH+stepH),position++,controller.getColorOfPos(position)));
                    }
                }
            }
            inited = true;
